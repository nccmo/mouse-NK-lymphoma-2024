---
title: "GSEA_BubblePlot_Trp53tumor_vs_Trp53LMP1tumor"
...
# pdf
outdir_pdf <- str_glue("result/REVISION/{tst}/4_Data_filt3/6_PathwayHeatmap/02_GSEA_BubblePlot_Trp53tumor_vs_Trp53LMP1tumor/pdf")
dir.create(outdir_pdf, recursive = TRUE, showWarnings = FALSE)

# table
outdir_table <- str_glue("result/REVISION/{tst}/4_Data_filt3/6_PathwayHeatmap/02_GSEA_BubblePlot_Trp53tumor_vs_Trp53LMP1tumor/table")
dir.create(outdir_table, recursive = TRUE, showWarnings = FALSE)

# Rds
outdir_Robj <- str_glue("result/REVISION/{tst}/4_Data_filt3/6_PathwayHeatmap/02_GSEA_BubblePlot_Trp53tumor_vs_Trp53LMP1tumor/rds")
dir.create(outdir_Robj, recursive = TRUE, showWarnings = FALSE)
```

```{r load GSEA result}
GSEA_result_genes_df <- read_tsv("result/REVISION/2023-08-29/4_Data_filt3/5_Pathway/02_GSEA_Trp53tumor_vs_Trp53LMP1tumor/table/df_GSEA_LMPnTumor_vs_LMPpTumor_celltype_overall_detail_with_genenames.txt.txt")

GSEA_result_genes_df$celltype_overall_detail <- factor(GSEA_result_genes_df$celltype_overall_detail,
                                                       levels = c("CD4T_N", "CD4T_EM", "TREG", 
                                                                  "CD8T_CM", "CD8T_EM", "CD8T_EX",
                                                                  "B_Immature", "B_Mature", 
                                                                  "Mono", "cDC1", "cDC2", "pDC", "MDSC",
                                                                  "NK"))
```
...
GSEA_result_genes_Escore04_qvaluetop10_df %>% 
  write_tsv(str_glue("{outdir_table}/df_GSEA_LMPn_vs_LMPpTumor_celltype_overall_detail_ES040_qval010_Top10.txt"))
```
...
df <- GSEA_result_genes_df %>% 
  ...
  filter(qvalue < 0.1) %>% 
  mutate(Comparison = "Trp53-/- tumor vs Trp53-/- LMP1+ tumor",
         `Tumor type` = case_when(enrichmentScore < 0 ~ "Trp53-/- tumor",
                                  enrichmentScore > 0 ~ "Trp53-/- LMP1+ tumor"),
...
df %>% 
  write_tsv(str_glue("{outdir_table}/df_GSEA_for_suppl_LMPn_vs_LMPp_qval01.txt"))
```
...
GSEA_result_genes_Escore04_qvaluetop10_df_select_myeloid %>% 
  ungroup() %>% 
  filter(micetype == "Trp53-/- LMP1+ tumor") %>% 
  write_tsv(str_glue("{outdir_table}/GSEA_LMPn_vs_LMPpTumor_result_genes_Escore04_qvalue_more1_myeloid.txt"))
...
GSEA_result_genes_Escore04_qvaluetop10_more2_df_myeloid %>% 
  write_tsv(str_glue("{outdir_table}/GSEA_LMPn_vs_LMPpTumor_result_genes_Escore04_qvalue_more2_myeloid.txt"))
...
S1 <- GSEA_result_genes_Escore04_qvaluetop10_df_select_myeloid %>% 
  ...
  labs(title = "[Myeloid] LMPn vs LMPp tumor") + 
...
pdf(str_glue("{outdir_pdf}/Fig5c_plot_bubble_GESA_LMPn_vs_LMPpTumor_Myeloid.pdf"), width = 7, height = 6)
S1
dev.off()
```
