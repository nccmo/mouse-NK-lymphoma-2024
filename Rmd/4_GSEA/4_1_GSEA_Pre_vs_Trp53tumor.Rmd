...
# pdf
outdir_pdf <- str_glue("result/REVISION/{tst}/4_Data_filt3/5_Pathway/01_GSEA_Pre_vs_Trp53tumor/pdf")
dir.create(outdir_pdf, recursive = TRUE, showWarnings = FALSE)

# table
outdir_table <- str_glue("result/REVISION/{tst}/4_Data_filt3/5_Pathway/01_GSEA_Pre_vs_Trp53tumor/table")
dir.create(outdir_table, recursive = TRUE, showWarnings = FALSE)

# Rds
outdir_Robj <- str_glue("result/REVISION/{tst}/4_Data_filt3/5_Pathway/01_GSEA_Pre_vs_Trp53tumor/rds")
...
mouseNK_data <- subset(x = mouseNK_data, idents = "Trp53-/- LMP1+ tumor", invert = TRUE)
...
rna.markers_all_forGprofiler5 %>%
  write_tsv(str_glue("{outdir_table}/DEG_pre_vs_LMPnTumor_celltype_overall_detail.txt"))
```
...
gsea_result_df %>% 
  write_tsv(str_glue("{outdir_table}/df_GSEA_pre_vs_LMPnTumor_celltype_overall_detail.txt"))

gsea_result_genes_df %>% 
  write_tsv(str_glue("{outdir_table}/df_GSEA_pre_vs_LMPnTumor_celltype_overall_detail_with_genenames.txt"))
```
...
gsea_result_df_edit %>% 
  ...
  write_tsv(str_glue("{outdir_table}/S8_mu_sc_non-m_pathway_pre_vs_T_LMPn.txt"))

gsea_result_df_edit %>% 
  filter(NES < 0) %>% 
  group_by(celltype_overall_detail) %>% 
  dplyr::arrange(NES, .by_group = TRUE) %>% 
  mutate(micetype = "Pre-tumor") %>% 
  write_tsv(str_glue("{outdir_table}/S8_mu_sc_non-m_pathway_pre_vs_T_Pre.txt"))
```
