...
# pdf
outdir_pdf <- str_glue("result/REVISION/{tst}/4_Data_filt3/3_reclutser/9_Reannotation/04_subclustering_B/pdf")
dir.create(outdir_pdf, recursive = TRUE, showWarnings = FALSE)

# table
outdir_table <- str_glue("result/REVISION/{tst}/4_Data_filt3/3_reclutser/9_Reannotation/04_subclustering_B/table")
dir.create(outdir_table, recursive = TRUE, showWarnings = FALSE)

# Rds
outdir_Robj <- str_glue("result/REVISION/{tst}/4_Data_filt3/3_reclutser/9_Reannotation/04_subclustering_B/rds")
...
mouseNK_subdata <- subset(x = mouseNK_data, idents = "B")
...
```{r load gene sets}
known_genes <- read_tsv("ref/cluster_ref_genes/genes_for_bubbleplot_subcluster_B.txt")
...
pdf(str_glue("{outdir_pdf}/UMAP_B_subcluster_notAnnotated_dim{dim}res00{res*100}.pdf"))
print(p1)
print(p1 + NoLegend())
print(p2)
print(p2 + NoLegend())
...
# UMAP gene markers
pdf(str_glue("{outdir_pdf}/UMAP_B_subcluster_known_genes_dim{dim}.pdf"))
...
# 4, Add annotation
```{r add annotation}
df_cluster_annot <- read_tsv("ref/cluster_annotation/REV_cluster_annotation_B_dim25_res002_240327.txt")
...
mouseNK_subdata@meta.data$B_subcluster <- factor(mouseNK_subdata@meta.data$B_subcluster, 
                                                       levels = unique(df_cluster_annot$B_subcluster))
mouseNK_subdata@meta.data$celltype_B <- factor(mouseNK_subdata@meta.data$celltype_B, 
                                                     levels = unique(df_cluster_annot$celltype_B))
```
...
Idents(mouseNK_subdata) <- mouseNK_subdata$celltype_B
fig4 <- VlnPlot(mouseNK_subdata, 
                features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
                ncol = 3, pt.size = 0)

pdf(str_glue("{outdir_pdf}/FigS5i_UMAP_B_subcluster_Annotated_dim{dim}res00{res*100}_raster.pdf"))
print(fig1)
print(fig1 + NoLegend())
print(fig2)
print(fig2 + NoLegend())
...
df_genes <- read_tsv("ref/cluster_ref_genes/genes_for_bubbleplot_REVISION/genes_for_bubbleplot_celltype_B.txt")
...
pdf(str_glue("{outdir_pdf}/FigS5j_Dotplot_celltype_B_for_manuscript_scaled.pdf"), height = 10)
print(fig1)
dev.off()
```
...
saveRDS(mouseNK_subdata, str_glue("{outdir_Robj}/mouseNK_subdata_B_after_annotation_dim{dim}res00{res*100}.rds"))

mouseNK_subdata@meta.data %>%
  as_tibble() %>%
  write_tsv(str_glue("{outdir_table}/metadata_mouseNK_subdata_B_after_annotation_dim{dim}res00{res*100}.txt"))
```
