...
# pdf
outdir_pdf <- str_glue("result/REVISION/{tst}/4_Data_filt3/3_reclutser/9_Reannotation/01_subclustering_T_NK/pdf")
dir.create(outdir_pdf, recursive = TRUE, showWarnings = FALSE)

# table
outdir_table <- str_glue("result/REVISION/{tst}/4_Data_filt3/3_reclutser/9_Reannotation/01_subclustering_T_NK/table")
dir.create(outdir_table, recursive = TRUE, showWarnings = FALSE)

# Rds
outdir_Robj <- str_glue("result/REVISION/{tst}/4_Data_filt3/3_reclutser/9_Reannotation/01_subclustering_T_NK/rds")
...
mouseNK_data$annotationSeurat1 <- factor(mouseNK_data$annotationSeurat1,
                                         levels = annotationSeurat1_NKmouse_filt2_level)
...
# 2. T/NK subset
```{r subset}
Idents(mouseNK_data) <- mouseNK_data$celltype1
Idents(mouseNK_data) %>% unique()

mouseNK_subdata <- subset(x = mouseNK_data, idents = "T_NK")
...
pdf(str_glue("{outdir_pdf}/UMAP_T_NK_subcluster_notAnnotated_dim{dim}res00{res*100}.pdf"))
print(p1)
print(p1 + NoLegend())
print(p2)
print(p2 + NoLegend())
...
# UMAP gene markers
pdf(str_glue("{outdir_pdf}/UMAP_T_NK_subcluster_known_genes_dim{dim}.pdf"))
...
# 4, Add annotation
```{r add annotation}
df_cluster_annot <- read_tsv("ref/cluster_annotation/REV_cluster_annotation_T_NK_dim30_res050_240325.txt")
...
mouseNK_subdata@meta.data$T_NK_subcluster <- factor(mouseNK_subdata@meta.data$T_NK_subcluster, 
                                                    levels = unique(df_cluster_annot$T_NK_subcluster))
mouseNK_subdata@meta.data$celltype_T_NK <- factor(mouseNK_subdata@meta.data$celltype_T_NK, 
                                                  levels = unique(df_cluster_annot$celltype_T_NK))
```

# 5. Save
```{r save UMAP}
fig1 <- DimPlot(mouseNK_subdata, reduction = 'umap', 
               group.by = "T_NK_subcluster",
               label = FALSE, repel = TRUE, 
               raster = TRUE, pt.size = 1.5, raster.dpi = c(600, 600)) + 
  labs(title = str_glue("RNA dim: {dim}, res: {res}")) +
  theme(plot.title = element_text(size = 15))

fig2 <- DimPlot(mouseNK_subdata, reduction = 'umap', 
               group.by = "celltype_T_NK",
               label = FALSE, repel = TRUE,
               raster = TRUE, pt.size = 1.5, raster.dpi = c(600, 600)) + 
  labs(title = str_glue("RNA dim: {dim}, res: {res}")) +
  theme(plot.title = element_text(size = 15))

pdf(str_glue("{outdir_pdf}/UMAP_T_NK_subcluster_Annotated_dim{dim}res00{res*100}_raster.pdf"))
print(fig1)
print(fig1 + NoLegend())
print(fig2)
print(fig2 + NoLegend())
...
saveRDS(mouseNK_subdata, str_glue("{outdir_Robj}/mouseNK_subdata_T_NK_after_annotation_dim{dim}res00{res*100}.rds"))

mouseNK_subdata@meta.data %>%
  as_tibble() %>% 
  write_tsv(str_glue("{outdir_table}/metadata_mouseNK_subdata_T_NK_after_annotation_dim{dim}res00{res*100}.txt"))
```
