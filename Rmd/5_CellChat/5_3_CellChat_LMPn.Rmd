...
# pdf
outdir_pdf <- str_glue("result/REVISION/{tst}/4_Data_filt3/14_CellInteraction/04_CellChat_LMPn/pdf")
dir.create(outdir_pdf, recursive = TRUE, showWarnings = FALSE)

# table
outdir_table <- str_glue("result/REVISION/{tst}/4_Data_filt3/14_CellInteraction/04_CellChat_LMPn/table")
dir.create(outdir_table, recursive = TRUE, showWarnings = FALSE)

# Rds
outdir_Robj <- str_glue("result/REVISION/{tst}/4_Data_filt3/14_CellInteraction/04_CellChat_LMPn/rds")
...
```{r subset}
# subset LMPn tumor samples
Idents(mouseNK_data) <- mouseNK_data$disease_type2_edit
mouseNK_data <- subset(x = mouseNK_data, idents = "Trp53-/- tumor", invert = FALSE)
...
df.net %>% 
  ...
  write_tsv(str_glue("{outdir_table}/df_inferred_cellular_communication_network_LR_LMPn_for_suppl.txt"))
```
...
df_info_flow %>% 
  write_tsv(str_glue("{outdir_table}/df_information_flow_LMPn_All.txt"))
```

# Part V: Save the CellChat object
```{r}
saveRDS(cellchat, file = str_glue("{outdir_Robj}/mouseNK_data_cellchat_LMPn.rds"))
```
